...
async def analyze_audio_api(file: UploadFile = File(...), output_lang: str = Form(...)):
    ...
    try:
        ...
        try:
            ...
            return JSONResponse(
                status_code=500,
                content={"error": f"Can't invoke '{bedrock_model_id}'. Reason: {e}"}
            )
        ...
        # Cleanup temp files
        if 'cleaned_audio_path' in locals() and cleaned_audio_path != tmp_path:
...
def upload_audio(request: Request, file: UploadFile = File(...), output_lang: str = Form(...)):
    # Your existing HTML upload logic remains the same
    # Save uploaded file to temp
    with tempfile.NamedTemporaryFile(delete=False, suffix=os.path.splitext(file.filename)[-1]) as tmp:
        shutil.copyfileobj(file.file, tmp)
        tmp_path = tmp.name
